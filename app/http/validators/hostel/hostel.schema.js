...
const addHostelSchema = Joi.object({
  ...
  hostelType: Joi.string().valid("private", "public").required()
    .error(createHttpError.BadRequest("نوع اسکان باید 'private' یا 'public' باشد")),
  ...
  hostelOwnNameJoi: string().min(3).max(100).required()
    .error(createHttpError.BadRequest("نام صاحب اسکان الزامی است و باید معتبر باشد")),
});
